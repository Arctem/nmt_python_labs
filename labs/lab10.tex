...
we create iterable. These two functions are \pythoninline{__iter__(self)} and
\pythoninline{__next__(self)}. These functions are used by anything that uses
...
\pythoninline{__next__} defined. Thus, we can usually just
\pythoninline{return self} if we've defined it ourselves. Then,
\pythoninline{__next__} is called repeatedly until it raises an exception called
\pythoninline{StopIteration}. Using this knowledge we can write our own version
of \pythoninline{range()}:
...
With a bit more complex logic in our \pythoninline{__next__} we can do some
...
\end{verbatim}
    ...
    Once you have it using, you should be able to use \pythoninline{list} to
