...
\begin{description}
  \item[rps.py] Write a program that reads a character for playing the game of
...
\end{verbatimcode}
  ...
  \item[sums.py] Write a program that keeps prompting the user for numbers to
...
%    \pythoninline!.format()! is still required.
  ...
  \item[primes.py] Write a program that checks if a number $N$ is prime. You
    have to ask the user for the number. Remember that a prime number is a
...
%  \item fizzbuzz\_for.py (see Section~\ref{subsec:whileex})
  ...
  \item rps.py (see Section~\ref{subsec:whileex})
  \item sums.py (see Section~\ref{subsec:whileex})
