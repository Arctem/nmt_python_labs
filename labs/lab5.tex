...
Dictionaries are similar to lists, but with a different index system. Unlike
with a list, the indices of a dictionary can be any immutable object. This
means that dictionaries are useful for storing elements best identified
by a description or name rather than a strict order. Each element in a
dictionary must be associated with an index, which is called a key. The
