...
\begin{description}
  \item[conversions.py] Use \texttt{conversions.py} from last time and add a
...
You entered a letter I do not recognize.
  ...
  \item[calculator.py] Write a small calculator that can compute arcsin, arccos,
    ...
    Make sure to check for each function that the input is valid. For example,
    for square root the input cannot be negative. For arcsin, the input must be
    between -1 and 1 inclusive. Try to figure out what the input must be for
    arccos and arctan yourself!
...
\begin{itemize}
  ...
  \item modulus.py (see Section~\ref{subsec:ifex})
  \item 
\end{itemize}
